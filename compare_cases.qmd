...
@render.data_frame
def cap_df():
    df = cap_cases().groupby(["model", "case", "tech_type", "agg_zone", "planning_year", "unit"], as_index=False)["end_value"].sum()
...
def gen_df():
    df = (
        gen_cases()
        .groupby(
            ["model", "case", "tech_type", "agg_zone", "planning_year", "unit"],
            as_index=False,
        )["value"]
        .sum()
    )
...
def chart_tx_expansion(
    ...
    if col_var is None or row_var is None:
        ...
        if not (col_var is None and row_var is None):
            _tooltips.append(alt.Tooltip("line_growth", format=".1%"))
