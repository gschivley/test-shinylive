...
CASE_NAME_MAP = {
    "full-base-200": "CO₂ cap",
    "full-base-200-retire": "CO₂ cap retire",
    "full-base-200-no-ccs": "CO₂ cap no CCS",
    "full-base-200-commit": "CO₂ cap unit commit",
    "full-base-50": "CO₂ cap $50",
    "full-base-1000": "CO₂ cap $1000",
    "full-base-200-tx-0": "CO₂ cap no tx",
    "full-base-200-tx-15": "CO₂ cap 15% tx",
    "full-base-200-tx-50": "CO₂ cap 50% tx",
    "20-week-foresight": "CO₂ cap 20-week foresight",
    "20-week-myopic": "CO₂ cap 20-week",
    "full-current-policies": "Current policy",
...
def op_costs_model_data():
    ...
    df = df.loc[
        (df["case"].isin(case_list()))
        & (df["planning_year"].isin(list(input.year()) + ["NPV"]))
        & (df["model"].isin(input.models())),
        :,
    ]
    df["case"] = df["case"].map(WRAPPED_CASE_NAME_MAP)
...
ui.input_switch("cap_zero_y", "Zero Y-Axis", True)
...
ui.input_selectize(
    "gen_line_col_var",
    "Column",
    choices=["case", "tech_type", "agg_zone", "None"],
    selected="tech_type",
    width="150px",
)
ui.input_selectize(
...
ui.input_switch("gen_line_independent_y_scale", "Independent Y-Axis", False)
```
```{python}
@render_altair
def alt_mean_gen_line():
    chart = chart_avg_gen_variation_line(
        # calc_mean_annual_gen(gen_cases(), by_agg_zone=True),
        calc_mean_annual_gen(),
        row_var=input.gen_line_row_var(),  # "agg_zone",
        # x_var=input.avg_x_var(),  # "case",
        col_var=input.gen_line_col_var(),
        color=input.gen_line_color(),
        independent_y_scale=input.gen_line_independent_y_scale(),
        # "tech_type",
        # order=order,
    )
    return chart
...
ui.input_selectize(
    "emiss_color",
    "Color",
    choices=["case", "planning_year", "model", "Region"],
    selected="Region",
    width="150px",
)
...
def chart_total_cap(
    ...
    # if col_var != "case":
    #     cap_data["case"] = cap_data["case"].str.replace("\n", "")
    chart = c.encode(
...
def chart_total_gen_line(
    ...
    data = (
        data.groupby(list(set(group_by)))[["value", "capacity_factor", "end_value"]]
        .mean()
        .reindex(midx)
        .reset_index()
    )
...
def chart_op_cost(
    ...
    if col_var == "case":
        final_chart = final_chart.configure(lineBreak="\n").configure_axis(
            labelFontSize=15, titleFontSize=15
        )
    return final_chart
